...
import {
  ...
  ConfigProvider, Grid, InputNumber, List, Typography
} from "antd";
    ...
    >
      <Layout style={{ minHeight: "100vh", background: "#f7f8fa" }}>
        ...
        <Layout style={{ minHeight: "calc(100vh - 64px)" }}>
          {/* Sidebar — список папок */}
          <Sider width={270} style={{
            background: "#f7f8fa",
            borderRight: "1px solid #eee",
            padding: "26px 0 0 0"
          }}>
            <div style={{ padding: "0 16px 18px 22px", display: "flex", gap: 6 }}>
              <Button type="primary" icon={<PlusOutlined />} onClick={openAdd} size="small">
                Добавить мок
              </Button>
              <Button icon={<PlusOutlined />} onClick={openAddFolderModal} size="small">
                Добавить папку
              </Button>
            </div>
            <List
              ...
              renderItem={folder => (
                <List.Item
                  key={folder}
                  style={{
                    background: folder === selectedFolder ? "#dde6fa" : undefined,
                    borderRadius: 6,
                    cursor: "pointer",
                    marginBottom: 2,
                    marginRight: 10,
                  ...
                  actions={folder !== "default"
                    ...
                    : []}
                >
                  <Typography.Text style={{fontWeight: folder === selectedFolder ? 700 : 400}}>{folder}</Typography.Text>
                </List.Item>
              )}
            />
          </Sider>
